...
public class MainActivity extends ActionBarActivity implements Callback {
	...
	public Handler myHandler = new Handler() {
		...
		private void draw() { 
			...
			{
				int iRssi = Math.abs(mRSSIVector[i][mRSSINum[i]-1]);
				float power = (float) ((iRssi-59)/(10*2.0));
				float dis=(float) Math.pow(10, power); 
				canvas.drawLine(0, 100*i+95, 320, 100*i+95, mPaint.get(i));
				canvas.drawText(mDevicesVector.get(i)+">>  RSSI: "+mRSSIVector[i][mRSSINum[i]-1].toString()+"  距离: "+ dis, 5, 100*i+107, mPaint.get(i));
				for(int j=0;j<mRSSINum[i];j++)
				{
					canvas.drawLine(j*5+5, 100*i+95 , j*5+5, 100*i+95+mRSSIVector[i][j], mPaint.get(i));
				}
			}
